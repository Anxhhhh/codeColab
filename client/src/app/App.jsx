...
const App = () => {
  ...
  const yTree = useMemo(() => ydoc.getMap("tree"), [ydoc]);
  ...
  useEffect(() => {
    const updateTree = () => {
      setTree(yTree.toJSON());
    };
    yTree.observeDeep(updateTree);
    updateTree();
    return () => yTree.unobserveDeep(updateTree);
  }, [yTree]);

  // Handle active file deletion
  useEffect(() => {
    if (activeFileId && !tree[activeFileId]) {
  ...
  // Monaco Binding
  useEffect(() => {
    if (!editorMounted || !editorRef.current || !activeFileId || !providerRef.current) return;

    let fileText = yFiles.get(activeFileId);
    if (!fileText) {
      fileText = new Y.Text();
      yFiles.set(activeFileId, fileText);
    }
  ...
  }, [editorMounted, activeFileId, yFiles]);
  ...
  return (
    <div className="flex h-screen w-screen bg-[#1e1e1e] text-[#cccccc] font-sans overflow-hidden">
      ...
      {/* Primary Sidebar */}
      <div className="w-64 border-r border-[#2d2d2d] bg-[#252526] flex flex-col shrink-0 z-10 overflow-hidden">
        
        {/* Explorer Section */}
        <div className="flex flex-col border-b border-[#2d2d2d]" style={{ flex: '1 1 50%', minHeight: 0 }}>
          <FileExplorer 
            yTree={yTree}
            yFiles={yFiles}
            tree={tree}
      ...
      <div className="flex-1 flex flex-col bg-[#1e1e1e] min-w-0">
        {/* Editor Tabs */}
        {activeFileId && tree[activeFileId] && (
          <div className="flex h-9 bg-[#2d2d2d] shrink-0 overflow-x-auto overflow-y-hidden items-end ps-0">
            <div className="bg-[#1e1e1e] text-[#cccccc] border-t-2 border-t-blue-500 px-4 py-1.5 text-[13px] flex items-center gap-2 cursor-pointer border-r border-[#2d2d2d] min-w-[120px] max-w-[200px]">
              <span className="text-yellow-400 font-medium text-xs border border-yellow-400/30 px-1 rounded-sm">
                {tree[activeFileId].name.split('.').pop().toUpperCase()}
              </span>
              <span className="truncate">{tree[activeFileId].name}</span>
            </div>
        ...
        <div className="flex-1 relative pt-2">
          {activeFileId ? (
            <Editor
              height="100%"
              onMount={handleMount}
              path={activeFileId} // Forces re-mount or fresh model when activeFileId changes if language isn't dynamic, actually standard is to set path
              defaultLanguage="javascript"
              language={tree[activeFileId]?.name?.endsWith('.css') ? 'css' : tree[activeFileId]?.name?.endsWith('.html') ? 'html' : 'javascript'}
              theme="vs-dark"
              options={{
            ...
            <div className="flex items-center justify-center h-full text-gray-500 flex-col gap-4">
              <span className="text-6xl opacity-20">codeColab</span>
              <span>Select or create a file to start collaborating.</span>
            </div>
          )}
        </div>
